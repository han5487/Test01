...
public class PDFUtil {
	...
	public PDFUtil(String url) {
		try {
			// sysFont = PdfFontFactory.createFont("STSongStd-Light", "UniGB-UCS2-H",
			// false);//中文设置
			sysFont = PdfFontFactory.createFont("C:/Windows/Fonts/simsun.ttc,1", PdfEncodings.IDENTITY_H);
	...
	public void addHeading1(Document doc, String pg) throws Exception {
		Paragraph paragraph = new Paragraph();
		paragraph.add(pg).setFont(sysFont).setBold().setFontSize(16).setTextAlignment(TextAlignment.CENTER);
		doc.add(paragraph);
	...
	public void addHeading2(Document doc, String pg) throws Exception {
		Paragraph paragraph = new Paragraph();
		paragraph.add(pg).setFont(sysFont).setBold().setFontSize(14).setTextAlignment(TextAlignment.CENTER);
		doc.add(paragraph);
		catalogMap.put(pg, doc.getPdfDocument().getNumberOfPages());
	}
